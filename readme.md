...
| 主界面/设置车票背景界面/铁路系统选择界面 | item-prevpage             | 上一页按钮                         |
| 主界面/设置车票背景界面/铁路系统选择界面 | item-nextpage             | 下一页按钮                         |
...
| 车站选择界面/铁路系统选择界面       | item-back                 | 返回上级菜单                        |
...
TCCoasters（TCC）云轨的道岔用**数字节点名**标记各分叉，此时进入方向直接写节点名，如 `[+train:1]`；多个节点名用**逗号**分隔，如 `[+train:1,3]`。方向字符与数字节点名不能混写。

**出向（第三、四行）**：格式 `出向@线路id`（`@` 两侧都不能为空）：

- **出向**：同样沿用 TrainCarts switcher 牌的写法——绝对方向 `e`/`s`/`w`/`n`、相对牌子的 `f`/`b`/`l`/`r`，或**道岔节点名**。**TCC 云轨道岔用数字节点名**，如 `1@pr-cw`。出向解析与原版 switcher 牌一致：先按道岔节点名匹配（普通铁轨的节点名就是 `n`/`e`/`s`/`w`，云轨是数字），再退回方向解析。
- **线路 id**：`railway_routes.yml` 中定义的线路 id。
- **共用轨道**：一个出向被多条线路共用时，`@` 后用分号分隔多个线路 id，如 `r@pr-cw;pr-s1`。在路由图里它等于多条边（各属一条线），物理上是同一出向。

选向逻辑：

1. 列车有导航序列时（持票/刷卡的直达车），按导航当前道岔步骤记录的**物理出向**直接选向。
2. 无导航序列的列车在「有正线的进站道岔」，按路由图结构判定的到发线出向选向（普通车 / 手动车一律走到发线进站停靠）。
3. 再回退：按线路属性 / tag 集合里第一个被命中的出向选向。
4. 都无匹配时保持默认方向（不切换）。
