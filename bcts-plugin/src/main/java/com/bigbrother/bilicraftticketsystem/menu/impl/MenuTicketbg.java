...
public class MenuTicketbg extends Menu {
    ...
    private MenuTicketbg(Player player) {
        ...
        FileConfiguration ticketbgConfig = MenuConfig.getTicketbgMenuConfig();
        gui = PagedGui.items()
                .setStructure(buildStructure(ticketbgConfig, itemName -> switch (itemName) {
                    case "sharedbg" -> Markers.CONTENT_LIST_SLOT_HORIZONTAL;
                    case "selfbg" -> {
                    ...
                    case "back" -> new BackToMainItem();
                    case "defaultbg" -> new DefaultbgItem();
                    case "sort" -> (sortItem = new SortItem());
                    default -> null;
                }))
                .build();
        this.window = buildSingleWindow(player, ticketbgConfig, gui);
